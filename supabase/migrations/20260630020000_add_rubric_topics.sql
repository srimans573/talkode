-- Canonical, LLM-categorized topic list for each assessment's rubric.
-- Computed once at assessment-creation time (regardless of rubric source —
-- uploaded or template) so every candidate session reports rubric_scores
-- against the same fixed set of topics, independent of how the rubric text
-- happens to be formatted.
alter table public.assessments
add column if not exists rubric_topics jsonb not null default '[]'::jsonb;
