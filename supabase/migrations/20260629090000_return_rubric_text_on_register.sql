...
begin
  ...
  -- The rubric is written directly onto the assessment row at creation time
  -- (either uploaded or filled from a default template) — read it from here
  -- rather than assessment_rubric_templates, which only holds the library of
  -- default templates keyed by codebase_template_id, not per-assessment rubrics.
  rubric_text := matched_assessment.rubric_text;
